...
public class YoneticiController {
    ...
    public ModelAndView login(@ModelAttribute("yonetici") Yonetici yonetici, HttpSession session, HttpServletResponse response) {
        ...
        for (Yonetici yonetici1 : yoneticiler) {
            if (yonetici1.getKullaniciadi().equals(yonetici.getKullaniciadi()) && yonetici1.getSifre().equals(yonetici.getSifre())) {
                session.setAttribute("user", yonetici1);
                Cookie cookie = new Cookie("yonetici", yonetici1.getKullaniciadi());
