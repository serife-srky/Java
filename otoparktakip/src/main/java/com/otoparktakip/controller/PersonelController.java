...
public class PersonelController {
    ...
    public ModelAndView login(@ModelAttribute("personel") Personel personel, HttpSession session, HttpServletResponse response) {
        ...
        for (Personel personel1 : personeller) {
            if (personel1.getKullaniciadi().equals(personel.getKullaniciadi()) && personel1.getSifre().equals(personel.getSifre())) {
                session.setAttribute("user", personel1);
                Cookie cookie = new Cookie("personel", personel.getKullaniciadi());
